...
function main(){
    ...
    var canvas = document.getElementById("canvas_20");
    var fpsText = document.getElementById("fps")
    var num = parseInt(canvas.getAttribute("num")) 
    var trange = parseFloat(canvas.getAttribute("trange"));
    ...
    for(var i=0; i<num; i++){
        ...
        objs.push({
            ...
            pos: {x: trange*random(), y: trange*random(), z: trange*random()} ,
            rot: {x: 3*Math.random(), y: 3*Math.random(), z: 3*Math.random()},
            scl: {x: 0.6, y: 0.6, z:0.6}
        });
    }
    ...
    function draw(now){
        ...
        frameTotal += fps
        frameCount++;
        const avg = frameTotal/frameCount;
        if(fpsText){
